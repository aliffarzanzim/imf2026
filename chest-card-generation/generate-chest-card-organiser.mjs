...
export function renderCard(delegate, qrDataUri) {
  const roleText = delegate.role || 'Organiser';
  return `
    <div class="chest-card">
      ...
      <div class="badge-middle-wrap">
        ...
        <div class="badge-verify-block">
          <div class="qr-frame">
            <img src="${qrDataUri}" alt="${roleText} QR Verification">
          </div>
          <div class="verify-details">
            <span class="verify-label">${roleText} ID</span>
  ...
  <style>
    *, *::before, *::after {
    ...
    .chest-card {
      width: 376px;
      height: 540px;
      background: linear-gradient(180deg, #edf7f2 0%, #f4faf6 40%, #e6f3eb 100%);
      border-radius: 18px;
      overflow: hidden;
      position: relative;
      display: flex;
    ...
    /* ============================================================
       TOP SECTION: Deep Forest (Lighter 20%) with Incomplete Flush Tab
       ============================================================ */
    .badge-header-block {
      background: linear-gradient(170deg, #083b29 0%, #0e563c 55%, #157351 100%);
      ...
      text-align: center;
      flex-shrink: 0;
      box-shadow: 0 6px 20px rgba(5, 36, 25, 0.35);
    }
    ...
    .partner-capsule {
      background: linear-gradient(180deg, #edf7f2 0%, #f4faf6 100%);
      padding: 4px 10px 5px 10px;
      border-radius: 0 0 12px 12px;
      border: 1px solid rgba(167, 207, 189, 0.8);
      border-top: none;
      display: inline-flex;
    ...
    /* Deep Forest Lower Border Band (Lighter 20%) */
    .badge-bottom-band {
      width: 100%;
      height: 14px;
      background: linear-gradient(90deg, #083b29 0%, #0e563c 50%, #157351 100%);
      flex-shrink: 0;
    }
...
if (isDirectRun) {
  const delegate = {
    id: 439,
    regNumber: "IMF-REG-0439",
    fullName: "Alif Farzan Zim",
  ...
  const fullHtml = renderFullHtml({
    title: `IMF 2026 Conference Badge - Organiser - ${delegate.regNumber} - ${delegate.fullName}`,
    heading: `IMF 2026 Chest Card (Organiser) &bull; ${delegate.fullName} (${delegate.regNumber})`,
    cardsHtml: cardHtml,
    isMultiPage: false
  });

  const outputPath = path.join(__dirname, 'chest-card-organiser.html');
  fs.writeFileSync(outputPath, fullHtml, 'utf8');
  console.log(`✓ Organiser chest card successfully generated for ${delegate.fullName} (${delegate.regNumber})!`);
  console.log(`Saved HTML to: ${outputPath}`);

  const pdfPath = path.join(__dirname, 'chest-card-organiser.pdf');
  const success = exportPdf(outputPath, pdfPath);
  if (success) {
    console.log(`✓ Press-Ready 100% Vector Organiser PDF successfully generated!`);

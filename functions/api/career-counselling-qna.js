...
async function ensureTableExists(db) {
  ...
  try {
    ...
    // Migrate any existing questions from career_club_questions if present
    try {
      await db.prepare(`
        INSERT OR IGNORE INTO career_counselling_questions (id, email, question, created_at, updated_at)
        SELECT id, email, question, created_at, updated_at FROM career_club_questions
      `).run();
    } catch (_) {}
  } catch (err) {
    console.error("Failed to ensure career_counselling_questions table:", err);
  }
}
...
export async function onRequestGet(context) {
  try {
    ...
    console.error("career-counselling-qna GET error:", err);
    return new Response(
...
export async function onRequestPost(context) {
  try {
    ...
    const insertResult = await env.DB.prepare(
      "INSERT INTO career_counselling_questions (email, question) VALUES (?, ?)"
    ...
    console.error("career-counselling-qna POST error:", err);
    return new Response(
...
export async function onRequestPut(context) {
  try {
    ...
    const existing = await env.DB.prepare(
      "SELECT id, email FROM career_counselling_questions WHERE id = ? LIMIT 1"
    ).bind(id).first();
    ...
    await env.DB.prepare(
      "UPDATE career_counselling_questions SET question = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
    ...
    console.error("career-counselling-qna PUT error:", err);
    return new Response(
...
export async function onRequestDelete(context) {
  try {
    ...
    await env.DB.prepare(
      "DELETE FROM career_counselling_questions WHERE id = ?"
    ).bind(id).run();
    ...
    console.error("career-counselling-qna DELETE error:", err);
    return new Response(
